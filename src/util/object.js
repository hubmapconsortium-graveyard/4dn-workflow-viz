...
export class TooltipInfoIconContainerAuto extends React.Component {
	...
	render() {
		...
		return <TooltipInfoIconContainer {...this.props} tooltip={tooltip} title={title || fallbackTitle || property}
...
export class CopyWrapper extends React.PureComponent {
	...
	render() {
		...
		if (isMounted && includeIcon) elemsToWrap.push(<i {...iconProps} key="copy-icon"
														  className="icon icon-fw icon-copy"
...
export const itemUtil = {
	...
	User: {
		...
		gravatar(email, size = null, props = {}, defaultImg = 'retro') {
			return <img title="Obtained via Gravatar" {...props}
