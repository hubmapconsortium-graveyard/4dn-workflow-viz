...
export function reduceProcessedFilesWithExperimentsAndSets(processed_files) {
	...
	return _.map(
		_.uniq(processed_files || [], false, function (pF) {
			return pF.accession;
		}),
		function (pF) {
			pF = _.clone(pF);
...
export function allFilesFromFileSetsInExperiment(experiment) {
	if (Array.isArray(experiment.filesets)) {
		return _(experiment.filesets).chain()
			.pluck('files_in_set')
			.filter(function (files_in_set) {
				return typeof files_in_set !== 'undefined';
			})
			.flatten(true)
			.value();
	}
...
export function groupExperimentsByBiosampleRepNo(experiments) {
	return _(experiments || []).chain()
		...
		.sortBy(function (expSet) {
			return parseInt(expSet[0]);
		}) // Sort outer list (biosamples) by bio_rep_no
...
export function groupExperimentsByBiosample(experiments) {
	return _(experiments || []).chain()
		...
		.sortBy(function (expSet) {
			return expSet[0];
		}) // Sort outer list (biosamples) by biosample id
		.map(function (expSet) {
			return expSet[1];
		}) // Creates [[expObjWBiosample1-1, expObjWBiosample1-2], [expObjWBiosample2-1, expObjWBiosample2-2], ...]
		.value();
}
