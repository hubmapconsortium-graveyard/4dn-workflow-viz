...
} from './../../../browse/components/table-commons';
...
export class ItemPageTable extends React.Component {
	...
	render() {
		var {
			results, loading, columnExtensionMap, columns, width, windowWidth,
			defaultOpenIndices, renderDetailPane, minWidth
		} = this.props;

		if (loading || !Array.isArray(results)) {
			return (
				<div className="text-center"
					 style={{paddingTop: 20, paddingBottom: 20, fontSize: '2rem', opacity: 0.5}}>
		...
		return (
			<div className={"table-row clearfix" + (typeof renderDetailPane !== 'function' ? ' no-detail-pane' : '')}>
				{_.map(columnDefinitions, (col, index) =>
					<div style={{'width': col.width}} className={"column column-for-" + col.field}
						 data-field={col.field} key={col.field || index}>
...
export class ItemPageTableLoader extends React.PureComponent {
	...
	constructor(props) {
		...
		this.state = {
			'items': null,
			'loading': true,
			'itemIndexMapping': _.object(_.map(props.itemUrls, function (url, i) {
				return [url, i];
			}))
		};
	}
	...
	render() {
		return React.cloneElement(this.props.children, _.extend({}, this.props, {
