...
export class ExperimentSetTables extends React.PureComponent {
	...
	render() {
		...
		if (loading || !Array.isArray(expSets)) {
			...
			return (
				<div className="text-center text-300 mt-1"
					 style={{paddingTop: 20, paddingBottom: 20, fontSize: '2rem', opacity: 0.5}}>
			...
			<div className="file-part-of-experiment-sets-container">
				...
				<ItemPageTable
					...
					renderDetailPane={(es, rowNum, width) =>
						<ExperimentSetDetailPane result={es} href={href} containerWidth={width || null}
												 windowWidth={windowWidth} paddingWidthMap={{
...
export class ExperimentSetTableTabView extends React.PureComponent {
	...
	render() {
		...
		return (
			<div>
				<ExperimentSetTablesLoadedFromSearch {...{requestHref, windowWidth, title, href}}
