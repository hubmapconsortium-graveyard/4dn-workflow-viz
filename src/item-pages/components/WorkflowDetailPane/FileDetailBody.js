...
export class FileDetailBody extends React.PureComponent {
	...
	fileTitleBox() {
		...
		if (Array.isArray(file)) { // Some sort of group (of files)
			...
			fileTitleFormatted =
				<a href={object.atIdFromObject(file) || '/' + file.uuid} className="inline-block">{fileTitle}</a>;
	...
	descriptionBox() {
		...
		return (
			<div className={"col-xs-12 col-lg-" + lgColSize + " box"}>
				<span
					className="text-600">{file.description ? 'Description' : (file.notes ? 'Notes' : 'Description')}</span>
	...
	qcBox() {
		...
		return (
			<div className="col-sm-6 col-lg-4 right box buttons-container">
				<ViewMetricButton file={qc} defaultBtnClassName="btn-secondary"
								  data-tip="View the Quality Control Metrics for this File"/>
			</div>
		);
