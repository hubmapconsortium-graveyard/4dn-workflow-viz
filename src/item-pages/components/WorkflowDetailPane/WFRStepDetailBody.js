...
export class WFRStepDetailBody extends React.PureComponent {
	...
	stepTitleBox() {
		...
		return (
			<div className="col-sm-6 box">
				...
				{stepHref ?
					<h3 className="text-500 text-ellipsis-container" data-tip={tipIfLongString(titleString)}><a
						href={stepHref}>{titleString}</a></h3>
					:
	...
	render() {
		...
		var listOfSoftwareInWorkflow = (wfr && wfr.workflow && Array.isArray(wfr.workflow.steps) &&
			_.any(wfr.workflow.steps, function (workflowStep) {
				return workflowStep.meta && workflowStep.meta.software_used;
			}) &&
			_.uniq(_.flatten(_.filter(
				_.map(wfr.workflow.steps, function (workflowStep) {
					return (workflowStep.meta && workflowStep.meta.software_used) || null;
				}),
				function (s) {
		...
		return (
			<div style={{minHeight: this.props.minHeight}}>
				<div className="information">
					...
					{workflow ? <hr/> : null}
					{workflow ?
						<WorkflowDetailsForWorkflowNodeRow workflow_run={wfr} step={step} workflow={workflow}/> : null}
					{listOfSoftwareInWorkflow ? <hr/> : null}
					{listOfSoftwareInWorkflow ?
						<SoftwareDetailsForWorkflowNodeRow software={listOfSoftwareInWorkflow}/> : null}
