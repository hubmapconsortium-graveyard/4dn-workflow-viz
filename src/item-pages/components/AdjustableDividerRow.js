...
export class DraggableVerticalBorder extends React.PureComponent {
	...
	render() {
		...
		return (
			<Draggable axis="x" position={{
				'x': xOffset,
				'y': 0
			}} {..._.pick(this.props, 'onStart', 'onStop', 'onDrag', 'bounds')}>
				<div className="draggable-border vertical-border" style={{'height': height, 'left': left - 5}}>
					<div className="inner">
						<div className="drag-handle"
							 style={{'height': handleHeight, 'top': (Math.max(height - handleHeight, 10) / 2)}}/>
					</div>
				</div>
			</Draggable>
		);
...
export class AdjustableDividerRow extends React.PureComponent {
	...
	render() {
		var {
			mounted, width, height, leftPanelCollapseWidth, minRightPanelWidth, minLeftPanelWidth, leftPanelDefaultSizeMD, leftPanelDefaultSizeLG,
			className, rightPanelClassName, leftPanelClassName, renderLeftPanel, renderRightPanel, leftPanelCollapseHeight, windowWidth
		} = this.props;
		...
		return (
			<div className={"row" + (className ? ' ' + className : '')}>
				<div
					className={"left-panel col-xs-12 col-md-" + leftPanelDefaultSizeMD + " col-lg-" + leftPanelDefaultSizeMD + (leftPanelClassName ? ' ' + leftPanelClassName : '')}
					style={{
						'width': (layoutSize === 'lg' || layoutSize === 'md') ? this.leftPanelWidth + xOffset : width + 20,
						height
					}}>
					...
					{(layoutSize === 'lg' || layoutSize === 'md') ?
						<DraggableVerticalBorder xOffset={xOffset} height={height || null} left={this.leftPanelWidth}
												 onStop={this.handleStopDrag} onDrag={this.handleDrag}
												 bounds={this.draggableBounds}/>
						: null}
				</div>
				<div
					className={"right-panel col-xs-12 col-md-" + rightPanelDefaultSizeMD + " col-lg-" + rightPanelDefaultSizeLG + (rightPanelClassName ? ' ' + rightPanelClassName : '')}
