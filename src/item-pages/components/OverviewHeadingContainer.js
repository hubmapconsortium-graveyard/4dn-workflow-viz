...
export class OverviewHeadingContainer extends React.Component {
	...
	static defaultProps = {
		...
		'prependTitleIconFxn': function (open, props) {
			return <i className={"expand-icon icon icon-" + (open ? 'minus' : 'plus')}
	...
	render() {
		...
		return (
			<div
				className={"overview-blocks-header" + (open ? ' is-open' : ' is-closed') + (typeof className === 'string' ? ' ' + className : '')}>
				...
				<Collapse in={open} onEnter={onStartOpen} onEntered={onFinishOpen} onExit={onStartClose}
						  onExited={onFinishClose} children={this.renderInner()}/>
			</div>
		);
	}
}
