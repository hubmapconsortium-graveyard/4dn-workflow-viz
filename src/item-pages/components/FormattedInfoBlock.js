...
import {ajax, console, isServerSide, object} from './../../util';
		...
		return (
			<div className={outerClassName}>
				...
				<div className="row">
					...
					<div className={"col-xs-10 col-lg-11" + (isSingleItem && noDetails ? ' no-more-details' : '')}
						 children={children}/>
				</div>
			</div>
		);
...
export class WrappedCollapsibleList extends React.Component {
	...
	itemRenderFxnFallback(item, idx, all) {
		return <WrappedListBlock className={this.props.itemClassName} context={item}
								 key={object.itemUtil.atId(item) || idx}/>;
	}
...
class FormattedInfoBlockList extends React.Component {
	...
	static propEndpointsValid(props) {
		return props.endpoints && Array.isArray(props.endpoints) && props.endpoints.length > 0 && _.every(props.endpoints, function (ep) {
		...
		if (loading) {
			...
			innerContent = (
				<div className="row loaded">
					{iconClass ? <div className="col-xs-2 col-lg-1 icon-container"><i className={"icon " + iconClass}/>
					</div> : null}
					<div
						className={"details-col " + (iconClass ? "col-xs-10 col-lg-11" : "col-sm-12") + (!detailContent && !children ? ' no-more-details' : '')}>
						{title ?
							titleHref ?
								<h5 className="block-title"><a href={titleHref} title={title}>{title}</a></h5>
								: <h5 className="block-title no-link">{title}</h5>
							: null}
						{detailContent || children ? <div className={"more-details " + extraDetailClassName}
														  children={detailContent || children}/> : null}
					</div>
				</div>
			);
		}
