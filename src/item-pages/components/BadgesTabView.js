...
export class BadgesTabView extends React.PureComponent {
	...
	static badgesByClassification = memoize(function (context, classificationPropMapping = BadgesTabView.classificationMap) {
		...
		const groupedBadges = _.groupBy(badges, function (badge) {
			return BadgesTabView.badgeClassification(badge, classificationPropMapping);
		});
	...
	render() {
		...
		const body = _.map(
			...
			function (badgeClassification) {
				...
				return (
					<div className={className} data-badge-classification={badgeClassification}
		...
		return (
			<div className="overflow-hidden">
				<h3 className="tab-section-title">
					<span
						className="text-400">{badgeListLen}</span>{" " + badgeSingularTitle + (badgeListLen > 1 ? 's' : '')}
				</h3>
	...
	render() {
		...
		if (parentID && object.itemUtil.atId(context) !== parentID) {
			...
			if (parentTypeTitle && parentDisplayTitle) { // These two props are default embeds. If not present is most likely due to lack of view permissions.
				titleToShow = <React.Fragment>{parentTypeTitle} <span
					className="text-600">{parentDisplayTitle}</span></React.Fragment>;
					...
					<div className="col-xs-12 col-sm-10 title-col">
						<div className="inner mb-05">
							<h4 className="text-500 mb-0 mt-0">
								{badgeTitle}
								{description ?
									<i className="icon icon-fw icon-info-circle ml-05" data-tip={description}/> : null}
							</h4>
							{renderedMessages}
							{linkMsg}
						</div>
