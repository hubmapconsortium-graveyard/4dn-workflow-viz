...
export class TopRow extends React.Component {
	...
	parsedStatus() {
		...
		return (
			<div className="indicator-item item-status" data-status={context.status.toLowerCase()}
	...
	itemActions() {
		...
		return _.map(
			...
			function (action, i) { // For each action, generate a clickable element.
				return (
					<div className="indicator-item action-button" data-action={action.name || null}
						 key={action.name || i}>
	...
	typeInfoLabel() {
		...
		return (
			<span className="type-info inline-block"
				  data-tip={(baseTypeInfo ? 'Base' : 'Abstract') + " type of this " + detailTitle + " Item"}>
                {title}
            </span>
		);
	}
			...
			<div className="row clearfix top-row">
				<h5 className="col-sm-5 item-label-title">
					<div className="inner">
						{this.typeInfoLabel()}
						{context.accession ?
							<object.CopyWrapper value={context.accession} className="accession inline-block"
												data-tip={accessionTooltip}
												wrapperElement="span"
	...
	parsedDate(dateToUse) {
		...
		return (
			<span data-tip={tooltip} className="inline-block">
				...
				<DateUtility.LocalizedTime timestamp={context[dateToUse]} formatType='date-time-md'
