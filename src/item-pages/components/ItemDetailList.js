						...
						if (
							...
							_.filter(objectWithAllItemKeys[rootKeys[i]][embeddedKeys[j]], function (v) {
								if (typeof v === 'string' || typeof v === 'number') {
									return false;
								} else if (v && typeof v === 'object' && _.keys(v).length < 2) {
									return false;
								} else {
									return true;
								}
			...
			(item) => {
				return _.map(columnKeys, (colKeyContainer, colKeyIndex) => {
					...
					if (Array.isArray(value)) {
						if (_.all(value, function (v) {
							...
							'value': value.map(function (v) {
								return Schemas.Term.toName(colKey, v);
							}).join(', '), 'key': colKey
						};
						if (_.any(value, function (v) {
							return typeof v === 'object' && v;
						}) && Array.isArray(colKeyContainer.childKeys)) { // Embedded list of objects.
					...
					if (typeof value === 'string' && value.length < 25) {
						return {
							'value': Schemas.Term.toName(colKey, value),
		...
		return (
			<div className="detail-embedded-table-container">
				<table className="detail-embedded-table">
					<thead>
					<tr>{
						[<th key="rowNumber" style={{
							minWidth: 36,
							maxWidth: 36,
							width: 36
						}}>#</th>].concat(columnKeys.map((colKeyContainer, colIndex) => {
										...
										hasChildren ? (() => {
												...
												return (
													<div style={{whiteSpace: "nowrap"}} className="sub-list-keys-header">{
														[<div key="sub-header-rowNumber"
															  className="inline-block child-list-row-number">&nbsp;</div>].concat(colKeyContainer.childKeys.map((ck) => {
															return (
																<div key={"sub-header-for-" + colKey + '.' + ck}
																	 className="inline-block" data-key={colKey + '.' + ck}
																	 ref={function (r) {
									...
									[<td key="rowNumber">{i + 1}.</td>].concat(row.map(function (colVal, j) {
										...
										if (Array.isArray(val) && val.length > 0 && !_.all(val, React.isValidElement)) {
											val = _.map(val, function (v, i) {
												return jsonify(v, columnKeys[j].key + ':' + i);
											});
										}
										return (
											<td key={("column-for-" + columnKeys[j].key)}
...
class DetailRow extends React.PureComponent {
	...
	render() {
		...
		if (labelNumber) {
			label = (
				<span>
                    <span className={"label-number right inline-block" + (this.state.isOpen ? ' active' : '')}><span
						className="number-icon text-200">#</span> {this.props.labelNumber}</span>
					{label}
                </span>
			);
		}
		...
		if (value.type === SubItemTitle) {
			...
			return (
				<div>
					<PartialList.Row label={label} children={value}
									 className={(className || '') + (this.state.isOpen ? ' open' : '')}/>
					<SubItemListView
			...
			return (
				<div className="array-group" data-length={item.length}>
					{React.Children.map(value.props.children, (c, i) =>
						<DetailRow
							{...this.props} label={i === 0 ? label : <span className="dim-duplicate">{label}</span>}
							labelNumber={i + 1} item={item[i]}
		...
		// Default / Pass-Thru
		return <PartialList.Row label={label} children={value}
								className={(className || '') + (this.state.isOpen ? ' open' : '')}/>;
	}
...
export class Detail extends React.PureComponent {
	...
	static formValue(item, popLink = false, keyPrefix = '', atType = 'ExperimentSet', columnDefinitions, depth = 0) {
		...
		if (item === null) {
			...
			if (SubItemTable.shouldUseTable(item)) {
				return <SubItemTable items={item} popLink={popLink} columnDefinitions={columnDefinitions}
									 parentKey={keyPrefix} atType={atType}/>;
			}
				...
				<ol>
					{item.length === 0 ? <li><em>None</em></li>
						...
						item.map(function (it, i) {
							return <li
								key={i}>{Detail.formValue(it, popLink, keyPrefix, atType, columnDefinitions, depth + 1)}</li>;
						})
					}
				</ol>
			);
			...
			if (linkElement) {
				...
				var releventProperties = _.object(
					_.map(_.filter(_.pairs(columnDefinitions), function (c) {
			...
			if (item.charAt(0) === '/' && item.indexOf('@@download') > -1) {
				...
				return <a key={item} href={item} target="_blank" download
						  rel="noreferrer noopener">{attach_title || item}</a>;
	...
	render() {
		...
		return <PartialList persistent={_.map(persistentKeys, this.renderDetailRow)}
							collapsible={_.map(collapsibleKeys, this.renderDetailRow)} open={open}/>;
	}
...
export class ItemDetailList extends React.Component {
	...
	seeMoreButton() {
		...
		return (
			<button type="button" className="item-page-detail-toggle-button btn btn-default btn-block"
