...
export function commonGraphPropsFromProps(props) {
	var graphProps = {
		...
		'renderDetailPane': function (selectedNode, paneProps) {
			return (
				<WorkflowDetailPane {...paneProps} {..._.pick(props, 'schemas', 'context', 'legendItems', 'windowWidth')}
...
export class WorkflowGraphSection extends React.PureComponent {
	...
	body() {
		...
		if (showChart === 'basic') {
			return <Graph {...this.commonGraphProps()} edgeStyle="curve"
						  columnWidth={mounted && width ? (width - 180) / 3 : 180}/>;
