	...
	render() {
		...
		return (
			<div className="alerts mt-2" {..._.omit(this.props, 'children', 'alerts')}
				 children={_.map(this.props.alerts, (alert, index, alerts) =>
					 <AlertItem {...{alert, index, alerts}} setDismissing={this.setDismissing}
								dismissing={this.state.dismissing} key={index}/>
				 )}/>
		);
	}
}
	...
	render() {
		...
		return (
			<Fade timeout={500}
				...
				<div
					className={"alert alert-dismissable alert-" + (alert.style || 'danger') + (alert.noCloseButton === true ? ' no-close-button' : '')}>
