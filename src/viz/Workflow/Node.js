...
export default class Node extends React.PureComponent {
	...
	static findCommonStepNodesInputInto() {
		var nodes = Array.from(arguments),
			nodeInputOfLists = _.filter(_.pluck(nodes, 'inputOf'), function (ioList) {
	...
	static isInputOfSameStep() {
		return Node.findCommonStepNodesInputInto(...arguments).length > 0;
	}
	...
	static isRelated(currentNode, selectedNode) {
		...
		if (selectedNode.name === currentNode.name || _.any((currentNode._source || []).concat(currentNode._target || []), function (s) {
	...
	static isFromSameWorkflowType(currentNode, selectedNode) {
		...
		if (typeof selectedNode.meta.workflow === 'string' && Array.isArray(currentNode._source)) {
			if (_.any(currentNode._source, function (s) {
		...
		if (typeof currentNode.meta.workflow === 'string' && Array.isArray(selectedNode._source)) {
			if (_.any(selectedNode._source, function (s) {
