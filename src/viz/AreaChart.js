...
export class StatsViewController extends React.PureComponent {
	...
	constructor(props) {
		...
		this.state = _.extend(
			...
			_.object(_.map(_.keys(props.searchURIs), function (k) {
				return ['resp' + k, null];
			}))
		);
	}
...
export class GroupByController extends React.PureComponent {
	...
	static defaultProps = {
		'groupByOptions': {
			...
			'experiments_in_set.experiment_type.display_title': <span><i
				className="icon icon-fw icon-bar-chart"/>&nbsp; Experiment Type</span>
		},
	...
	render() {
		var {children} = this.props,
			...
			childProps = _.extend(_.omit(this.props, 'children', 'initialGroupBy'), {
				currentGroupBy,
...
export class GroupByDropdown extends React.PureComponent {
	...
	render() {
		...
		const selectedValueTitle = loadingStatus === 'loading' ?
			<i className="icon icon-fw icon-spin icon-circle-o-notch"/> : groupByOptions[currentGroupBy];

		return (
			<div className={outerClassName}>
				<span className="text-500">{title}</span>
				<DropdownButton id={id} title={selectedValueTitle} onSelect={this.onSelect} style={buttonStyle}
								disabled={optionItems.length < 2}>
					{optionItems}
				</DropdownButton>
				{children}
			</div>
	...
	render() {
		...
		return (
			<div className="legend mb-27">
				<div className="row">{_.map(_.sortBy(_.pairs(colorScaleStore), function ([term, color]) {
					return term.toLowerCase();
				}), this.renderColorItem)}</div>
			</div>
		);
	}
			...
			<div className="chart-tooltip" style={_.extend(_.pick(margin, 'left', 'top'), {
				...
				<div className="line-notch" style={{'top': topPosition}}>
					{chartWidth && topPosition < chartHeight ? [
						<div key="before" className="horiz-line-before"
							 style={{'width': (leftPosition - 5), 'left': -(leftPosition - 5)}}/>,
						<div key="after" className="horiz-line-after"
							 style={{'right': -((chartWidth - leftPosition) - 4)}}/>
					] : null}
				</div>
...
export class AreaChart extends React.PureComponent {
	...
	static calculateXAxisExtents(mergedData, xDomain) {
		...
		if (xDomain && xDomain[0]) {
			...
			xExtents[0] = d3.min(mergedData, function (d) {
				return d.date;
			});
		}
		...
		if (xDomain && xDomain[1]) {
			...
			xExtents[1] = d3.max(mergedData, function (d) {
				return d.date;
			});
		}
	...
	static calculateYAxisExtents(mergedData, yDomain) {
		...
		if (yDomain && typeof yDomain[0] === 'number') {
			...
			yExtents[0] = d3.min(mergedData, function (d) {
				return d.total;
			});
		}
		...
		if (yDomain && typeof yDomain[1] === 'number') {
			...
			yExtents[1] = d3.max(mergedData, function (d) {
				return d.total;
			});
		}
	...
	commonDrawingSetup() {
		...
		const area = d3.area()
			.x(function (d) {
				return x(d.date || d.data.date);
			})
			.curve(curveFxn)
			//.x0 ( function(d){ return x(d.date || d.data.date);  } )
			//.x1 ( function(d){ return x(d.date || d.data.date) + 10;  } )
			.y0(function (d) {
				return Array.isArray(d) ? y(d[0]) : y(0);
			})
			.y1(function (d) {
				return Array.isArray(d) ? y(d[1]) : y(d.total || d.data.total);
			});
	...
	updateTooltip(evt) {
		...
		let dateFormatFxn = function (aDate) {
			return DateUtility.format(aDate, 'date-sm');
		};
			...
			'contentFxn': function (tProps, tState) {
				...
				if (termChildren.length > maxTermsVisible) {
					var lastTermIdx = maxTermsVisible - 1,
						currentActiveItemIndex = _.findIndex(termChildren, function (c) {
				...
				return (
					<div className={"label-bg" + (isToLeft ? ' to-left' : '')}>
						<h5 className={"text-500 mt-0 clearfix" + (isEmpty ? ' mb-0' : ' mb-11')}>
							{dateString}{total ? <span className="text-700 text-large pull-right"
													   style={{marginTop: -2}}>&nbsp;&nbsp; {total}</span> : null}
						</h5>
						{!isEmpty ?
										...
										<tr key={c.term || i} className={currentTerm === c.term ? 'active' : null}>
											<td className="patch-cell">
												<div className="color-patch"
													 style={{'backgroundColor': c.noColor ? 'transparent' : colorScale(c.term)}}/>
											</td>
	...
	render() {
		...
		return (
			<div className="area-chart-inner-container" onMouseMove={this.updateTooltip}
