...
import {Filters, console, navigate} from '../util';
...
export default class QuickInfoBar extends React.PureComponent {
	...
	renderStats(extraClassName = null) {
		...
		if (current && (typeof current.experiment_sets === 'number' || typeof current.experiments === 'number' || typeof current.files === 'number')) {
			stats = {
				'experiment_sets': <span>{current.experiment_sets}
					<small> / {total.experiment_sets || 0}</small></span>,
				'experiments': <span>{current.experiments}
					<small> / {total.experiments || 0}</small></span>,
				'files': <span>{current.files}
					<small> / {total.files || 0}</small></span>
			};
		} else {
			stats = {
		...
		return (
			<div className={"left-side clearfix" + (extraClassName ? ' ' + extraClassName : '')}>
				<Stat {...statProps} shortLabel="Experiment Sets" longLabel="Experiment Sets" classNameID="expsets"
					  value={stats.experiment_sets} key="expsets"/>
				<Stat {...statProps} shortLabel="Experiments" longLabel="Experiments" classNameID="experiments"
					  value={stats.experiments} key="experiments"/>
				<Stat {...statProps} shortLabel="Files" longLabel="Files in Experiments" classNameID="files"
					  value={stats.files} key="files"/>
				<div className="any-filters glance-label" data-tip={areAnyFiltersSet ? "Filtered" : "No Filters Set"}
		...
		return (
			<div className="col-xs-4 text-right browse-base-state-toggle-container">
				<div className="inner-more">
					<Toggle disabled={isLoadingChartData || this.state.togglingBrowseState}
							id="toggle-external-data-switch"
			...
			return (
				<div className="bottom-side">
					...
					<div className="graph-icon"
						 onMouseEnter={null /*_.debounce(()=>{ this.setState({ show : 'mosaicCharts' }); },1000)*/}>
		...
		return (
			<div className={"stat stat-" + classNameID} title={longLabel}>
				<div id={id + '-stat-' + classNameID} className={"stat-value" + (isLoadingChartData ? ' loading' : '')}>
					{isLoadingChartData ?
						<i className="icon icon-fw icon-spin icon-circle-o-notch" style={{opacity: 0.25}}/> : value}
				</div>
