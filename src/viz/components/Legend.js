		...
		return (
			<div className="term text-ellipsis-container">
				...
				>
					...
					{this.props.aggregateType && this.props[this.props.aggregateType] ?
						<span className="text-300"> &nbsp;({this.props[this.props.aggregateType]})</span> : null}
                </span>
			</div>
		);
	}
	...
	render() {
		return (
			<div className="field" data-field={this.props.field}
				 onMouseLeave={vizUtil.unhighlightTerms.bind(this, this.props.field)}>
...
class LegendViewContainer extends React.Component {
	...
	showToggleIcon() {
		return this.props.expandable && this.props.field.terms && this.props.field.terms.length > this.props.expandableAfter;
	}
	...
	toggleIcon() {
		...
		return (
			<div className="expand-toggle text-center" onClick={this.props.onToggleExpand}
				 data-tip={this.props.expanded ? "Collapse" : "Expand"} data-place="left">
	...
	static barPlotFieldDataToLegendFieldsData(field, sortBy = null, colorCycler = barplot_color_cycler) {
		if (Array.isArray(field) && field.length > 0 && field[0] && typeof field[0] === 'object') {
			return field.map(function (f) {
				return Legend.barPlotFieldDataToLegendFieldsData(f, sortBy);
			});
		}
