	...
	render() {
		...
		return (
			<div className={className} ref={this.barSectionElemRef}
				 ...
				 onMouseEnter={(e) => {
					 typeof onMouseEnter === 'function' && onMouseEnter(d, e);
				 }}
				 onMouseLeave={(e) => {
					 typeof onMouseLeave === 'function' && onMouseLeave(d, e);
				 }}
				 onClick={(e) => {
					 typeof onClick === 'function' && onClick(d, e);
				 }}
			/>
		);
	}
...
class Bar extends React.PureComponent {
	...
	renderBarSection(d, i, all) {
		var {
				hoverTerm, hoverParentTerm, selectedTerm, selectedParentTerm, onBarPartClick,
				onBarPartMouseEnter, onBarPartMouseOver, onBarPartMouseLeave,
				aggregateType, canBeHighlighted
			} = this.props,
			key = d.term || d.name || i,
		...
		return (
			<BarSection {...{isHoveredOver, isSelected, key, aggregateType, canBeHighlighted}} data-key={key} node={d}
						onClick={onBarPartClick} onMouseEnter={onBarPartMouseEnter} onMouseLeave={onBarPartMouseLeave}
...
export class ViewContainer extends React.Component {
	...
	renderBars() {
		...
		return _.map(bars.sort(function (a, b) { // key will be term or name, if available
				...
				<CSSTransition classNames="barplot-transition" unmountOnExit timeout={{enter: 10, exit: 750}}
							   key={d.term || d.name || i}>
					<Bar key={d.term || d.name || i} node={d}
						 ...
						 onBarPartMouseEnter={onNodeMouseEnter} onBarPartMouseLeave={onNodeMouseLeave}
						 onBarPartClick={onNodeClick}/>
				</CSSTransition>
		);
	}
