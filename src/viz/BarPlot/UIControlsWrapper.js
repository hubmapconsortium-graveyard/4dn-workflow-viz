...
export class UIControlsWrapper extends React.PureComponent {
			...
			<div className="show-type-change-section">
				<h6 className="dropdown-heading">
					<span className="inline-block"
						  data-tip={isSelectedDisabled ? "Enable some filters to enable toggling between viewing all and selected items." : null}>Show</span>
				</h6>
				<DropdownButton
					...
					children={this.renderDropDownMenuItems([
						...
						['filtered', <span className="inline-block" data-place="left"
										   data-tip={isSelectedDisabled ? 'No filters currently set' : null}>
				...
				<DropdownButton
					...
					title={(() => {
						...
						if (this.props.isLoadingChartData) {
							return <span style={{opacity: 0.33}}><i
								className="icon icon-spin icon-circle-o-notch"/></span>;
						}
	...
	render() {
		var {
			barplot_data_filtered, barplot_data_unfiltered, barplot_data_fields, isLoadingChartData,
						...
						<div className="x-axis-right-label">
							<div className="row">
								...
								<div className="col-xs-9 pull-right"
									 style={{"width": (layout.gridContainerWidth(windowWidth) * (windowGridSize !== 'xs' ? 0.25 : 1)) + 5 - 52}}>
									<DropdownButton
...
export class AggregatedLegend extends React.Component {
	...
	static collectSubDivisionFieldTermCounts = memoize(function (rootField, aggregateType = 'experiment_sets') {
		...
		retField.terms = _.object(_.sortBy(_.pairs(retField.terms), function (termPair) {
			return termPair[1][aggregateType];
		}));
	...
	getFieldForLegend() {
		...
		return Legend.barPlotFieldDataToLegendFieldsData(
			...
			function (term) {
				return typeof term[aggregateType] === 'number' ? -term[aggregateType] : 'term';
			}
		);
	}
