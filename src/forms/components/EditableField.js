...
export class EditableField extends React.PureComponent {
			...
			return (
				<span className="help-block">
					...
					{_.map(serverErrors, (err, i) =>
						<div
							key={'error-' + i}>{(serverErrors.length === 1 ? '' : (i + 1) + '. ') + err.description}</div>
					)}
                </span>
			);
		}
	...
	renderActionIcon(type = 'edit') {
		...
		switch (type) {
			case 'edit' :
				...
				return (
					<a href={"#edit-" + this.props.labelID} className={extClass + " edit-button"}
					   onClick={this.enterEditState} title="Edit">
			...
			case 'save' :
				...
				return (
					<a href={"#save-" + this.props.labelID} className={extClass + " save-button"}
					   onClick={this.saveEditState} title="Save">
	...
	renderEditing() {
		...
		if (style == 'inline') {
			...
			return (
				<span ref={absoluteBox ? this.fieldRef : null}
