...
export class BuildField extends React.PureComponent {
	...
	displayField(fieldType) {
		...
		switch (fieldType) {
				...
				if (field === 'aliases') {
					return (
						<div className="input-wrapper">
							<AliasInputField {...inputProps} onAliasChange={this.handleAliasChange}
			...
			case 'html':
			case 'code':
				return <FormControl type="text" inputMode="latin" {...inputProps} componentClass="textarea" rows={8}
									wrap="off"
									style={{'fontFamily': "Source Code Pro, monospace", 'fontSize': 'small'}}/>;
			case 'integer'          :
				return <FormControl type="number" {...inputProps} step={1}/>;
			case 'number'           :
				return <FormControl type="number" {...inputProps} />;
			case 'boolean'          :
				return (
					<Checkbox {..._.omit(inputProps, 'value', 'placeholder')} checked={!!(value)}
							  className="mb-07 mt-07">
			...
			case 'linked object'    :
				return <LinkedObj key="linked-item" {...this.props}/>;
			case 'array'            :
				return <ArrayField {...this.props} pushArrayValue={this.pushArrayValue} value={value || null}
								   roundTwo={roundTwo}/>;
			case 'object'           :
				return <div style={{'display': 'inline'}}><ObjectField {...this.props}/></div>;
			case 'attachment'       :
				return <div style={{'display': 'inline'}}><AttachmentInput {...this.props}/></div>;
			case 'file upload'      :
				return <S3FileInput {...this.props} />;
		}
		// Fallback
	...
	labelTypeDescriptor() {
		...
		return <div className="field-descriptor">{required ?
			<span style={{'color': '#a94442'}}> Required</span> : null}</div>;
	}
				...
				<div className="row">
					<div className="col-sm-12 col-md-4">
						<h5 className="submission-field-title text-ellipsis-container">
							...
							{fieldTip ?
								<InfoIcon className="mr-07" title={title} fieldType={fieldType}
										  schema={schema}>{fieldTip}</InfoIcon>
								: null}
							<span>{title}</span>
						</h5>
					</div>
	...
	render() {
		...
		return wrapFunc(
			<React.Fragment>
				<div
					className={'field-column col-xs-' + (excludeRemoveButton ? "12" : "10") + extClass}>{fieldToDisplay}</div>
				{excludeRemoveButton ? null : <SquareButton show={showDelete} disabled={disableDelete}
...
const SquareButton = React.memo(function SquareButton(props) {
	...
	return (
		<div className="col-xs-2 remove-button-column" style={style}>
			<Fade in={show}>
				<div
					className={"pull-right remove-button-container" + (!show ? ' hidden' : '') + (buttonContainerClassName ? ' ' + buttonContainerClassName : '')}>
					<Button tabIndex={2} bsStyle={bsStyle} disabled={disabled} onClick={onClick} data-tip={tip}><i
						className={"icon icon-fw icon-" + icon}/></Button>
				</div>
			</Fade>
		</div>
	);
			...
			childWindowAlert = function () {
				...
				return {
					...
					'message': (
						<div>
							<p class="mb-0">
								Please either <b>drag and drop</b> an Item (row) from this window into the submissions
								window or click its corresponding select (checkbox) button.
							</p>
							<p class="mb-0">You may also browse around and drag & drop a link into the submissions
								window as well.</p>
						</div>
					),
					'style': 'info'
			...
			<React.Fragment>
				<div className="linked-object-text-input-container row flexrow">
					<div className="field-column col-xs-10">
						<input onChange={this.handleTextInputChange} className={"form-control" + extClass}
							   inputMode="latin" type="text"
							   placeholder="Drag & drop Item from the search view or type in a valid @ID."
							   value={this.state.textInputValue} onDrop={this.handleDrop}/>
					</div>
				...
				return (
					<div className="submitted-linked-object-display-container text-ellipsis-container">
						...
						<a href={value} target="_blank"
						   ...
						   children={thisDisplay}/>
						&nbsp;<i style={{'marginLeft': 5, 'fontSize': '0.85rem'}}
								 className="icon icon-fw icon-external-link"/>
					</div>
				);
			} else {
				...
				if (keyComplete[intKey]) {
					...
					return (
						<div className="incomplete-linked-object-display-container text-ellipsis-container">
							...
							<a href="#" onClick={this.setSubmissionStateToLinkedToItem}
							   data-tip="Continue editing/submitting" children={thisDisplay}/>
							&nbsp;<i style={{'marginLeft': 5, 'fontSize': '0.85rem'}}
									 className="icon icon-fw icon-pencil"/>
						</div>
					);
				}
			}
...
const PreviewField = React.memo(function PreviewField(props) {
	...
	return (
		<div className="preview-field-container">
			<FormControl onChange={onChange} id={"field_for_" + field} name={field} value={value} type="text"
						 inputMode="latin" componentClass="textarea" rows={8}
...
class ArrayField extends React.Component {
	...
	generateAddButton() {
		...
		return (
			<div className="add-array-item-button-container">
				<Button bsSize={values.length > 0 ? 'small' : null} onClick={this.props.pushArrayValue}><i
					className="icon icon-fw icon-plus"/> Add</Button>
			</div>
		);
	}
	...
	render() {
		...
		const valuesToRender = _.map(values.length === 0 ? [null] : values, function (v, i) {
			return [v, schema, i];
		});
	...
	render() {
		...
		return (
			<div style={{'display': 'inherit'}}>
				<input id={"field_for_" + this.props.field} type='file' onChange={this.handleChange}
					   style={{'display': 'none'}} accept={this.acceptedTypes()}/>
				<Button>
	...
	render() {
		...
		return (
			<div>
				<div>
					<input id={"field_for_" + this.props.field} type='file' onChange={this.handleChange}
						   disabled={disableFile} style={{'display': 'none'}}/>
					<Button disabled={disableFile} style={{'padding': '0px'}}>
						<label children={filename_text} className="text-400" htmlFor={"field_for_" + this.props.field}
							   style={{
								   'paddingRight': '12px',
								   'paddingTop': '6px',
								   'paddingBottom': '6px',
					...
					<Fade in={showDelete}>
						<div className="pull-right">
							<Button tabIndex={2} bsStyle="danger" disabled={!showDelete} onClick={this.deleteField}><i
								className="icon icon-fw icon-times"/></Button>
						</div>
					</Fade>
				</div>
				{statusTip ?
				...
				{this.state.percentDone !== null ?
					<div className="row" style={{'paddingTop': '10px'}}>
						<div className="col-sm-3" style={{'float': 'left'}}>
							<a href="" style={{'color': '#a94442', 'paddingLeft': '10px'}} onClick={this.cancelUpload}
							   title="Cancel">
								{'Cancel upload'}
							</a>
						</div>
			...
			firstPartSelect = (
				<DropdownButton className="alias-lab-select form-control alias-first-part-input" id="firstPartSelect"
					...
					{_.map(submits_for_list, (lab) =>
						<DropdownItem key={lab.name} eventKey={lab.name}><span
							className="text-500">{lab.name}</span> ({lab.display_title})</DropdownItem>
					)}
				</DropdownButton>
			);
		} else { // Only 1 submits_for lab or 0 submits_for -- fallback to staticy thingy
			firstPartSelect =
				<InputGroup.Addon className="alias-lab-single-option">{currFirstPartValue}</InputGroup.Addon>;
		}
		return (
...
export function isValueNull(value) {
	...
	if (typeof value === 'object') {
		...
		else if (_.every(keys, function (k) {
