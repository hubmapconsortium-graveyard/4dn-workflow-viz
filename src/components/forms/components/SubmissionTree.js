...
class SubmissionProperty extends React.Component {
	...
	render() {
		...
		return (
			<div key={bookmark}
				 className={"submission-nav-leaf linked-item-type-name leaf-depth-" + depth + (isRequired ? ' is-required' : '') + (children.length > 0 ? ' has-children' : '')}>
				<div className="clearfix inner-title">
					<i className={"icon property-expand-icon clickable icon-" + (this.state.open ? 'minus' : 'plus')}
					   onClick={this.handleToggle}/>
					<span>{children.length} {bookmark || field}</span>
				</div>
				{children.length > 0 ? <Collapse in={this.state.open}>
...
class SubmissionLeaf extends React.Component {
	...
	render() {
		...
		return (
			<div
				className={"submission-nav-leaf linked-item-title leaf-depth-" + (this.props.depth) + (isCurrentlySelected ? ' active' : '')}>
				...
				{placeholders && placeholders.length > 0 ?
					<div className="list-of-properties" children={placeholders}/> : null}
			</div>
		);
	}
}
...
class InfoIcon extends React.Component {
	...
	render() {
		...
		return (
			<i style={{"marginLeft": "6px", 'fontSize': '0.8em'}}
			   className={"icon icon-info-circle" + (this.props.className ? ' ' + this.props.className : '')}
			   data-place="right" data-html={true} data-tip={this.props.children}/>
		);
	}
}
