...
import {
	getElementTop,
	animateScrollTo,
	getScrollingOuterElement,
	getPageVerticalScrollPosition
} from './../../util/layout';
		...
		if (typeof link === 'string' && link.length > 0) {
			title = (
				<div className="title-link-wrapper">
					{collapsibleButton}
					<a className={depth === 0 ? 'text-500' : 'text-400'}
					   href={(link.charAt(0) === '/' ? '' : '#') + link} onClick={(e) => {
						e.preventDefault();
						this.handleClick();
					}}>{title}</a>
				</div>
...
class TableEntryChildren extends React.Component {
	...
	render() {
		...
		return <ol className="inner" style={{'listStyleType': this.props.listStyleTypes[(this.props.depth || 0) + 1]}}
				   children={children}/>;
	}
}
...
export class TableOfContents extends React.Component {
	...
	/** @deprecated */
	static slugifyReactChildren(children) {
	...
	static textFromReactChildren(children) {
		...
		if (Array.isArray(children) && children.length > 0) {
			...
			var childPrimaryElemIfAny = _.find(childrenWithChildren, function (c) {
				return c && typeof c === 'object' && c.props && (c.type === 'code' || c.type === 'strong' || c.type === 'b');
			});
		...
		return (
			<div key="toc" className={"table-of-contents" + (this.state.widthBound ? ' width-bounded' : '')} style={{
				...
				{!isEmpty ?
					...
					: null}
				{includeNextPreviousPages ?
					<NextPreviousPageSection context={context} windowInnerWidth={windowWidth}/> : null}
			</div>
		);
	}
				...
				<div className="row">
					{context.previous ?
						<div className={"previous-section text-right col-xs-6"}>
							<h6 className="text-400 mb-02 mt-12"><i
								className="icon icon-fw icon-angle-left"/> {previousTitle}</h6>
							<h6 className="text-500 mt-0"><a
								href={context.previous['@id'] || '/' + context.previous.name}>{context.previous.display_title}</a>
							</h6>
						</div>
						: null}
					{context.next ?
						<div className={"next-section col-xs-6 pull-right"}>
							<h6 className="text-400 mb-02 mt-12">{nextTitle} <i
								className="icon icon-fw icon-angle-right"/></h6>
							<h6 className="text-500 mt-0"><a
								href={context.next['@id'] || '/' + context.next.name}>{context.next.display_title}</a>
							</h6>
						</div>
						: null}
				</div>
...
export class MarkdownHeading extends React.PureComponent {
	...
	static getAttributes(children) {
		...
		let childrenOuterText = _.filter(children, function (c) {
...
export class HeaderWithLink extends React.PureComponent {
	...
	render() {
		...
		return React.createElement(this.props.type || 'h2', _.omit(this.props, 'type', 'children', 'link', 'context'), [
			this.props.children,
			<i key="icon-link" className="icon icon-fw icon-link" onClick={this.handleLinkClick}
