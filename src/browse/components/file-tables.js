...
class FileColumnActionsBtn extends React.PureComponent {
		...
		if (otherBtnsExist) {
			...
			return (
				<div className="inline-block" style={{'position': 'relative', 'zIndex': 2}}>
					<Button bsSize="xs" className="in-stacked-table-button" bsStyle="primary"
	...
	juiceboxButton() {
		...
		return (
			<DropdownItem data-tip="Visualize this file in TCGA's JuiceBox Browser" onClick={onClick} key="juicebox"
						  className="text-left">
	...
	epigenomeButton() {
		...
		return (
			<DropdownItem data-tip="Visualize this file in WashU Epigenome Browser" onClick={onClick} key="epigenome"
						  className="text-left">
...
export function renderFileQCReportLinkButton(file, field, detailIndex, fileEntryBlockProps) {
	...
	return (
		<Button bsStyle="primary" bsSize="xs" data-tip={"View report - " + filename} href={file.quality_metric.url}
...
export class RawFilesStackedTable extends React.PureComponent {
			...
			case 'replicate' :
				return [
					...
					{columnClass: 'experiment', className: 'text-left', title: 'Experiment', initialWidth: 145},
					{
						...
						title: 'File Group',
						initialWidth: 30,
						visibleTitle: function (stackedBlockProps) {
							if (stackedBlockProps.selectedFiles && Array.isArray(stackedBlockProps.allFiles)) {
			...
			default:
				return [
					...
					{columnClass: 'experiment', className: 'text-left', title: 'Experiment', initialWidth: 145},
					{
						...
						title: 'File',
						initialWidth: 125,
						render: renderFileTitleColumn,
						visibleTitle: renderFileHeaderWithCheckbox
					}
				];
		}
		...
		return [
			{
				...
				initialWidth: 110,
				field: "quality_metric.Total Sequences"
			}
		];
	}
	...
	renderExperimentBlock(exp, i) {
			...
			contents = contents.concat(_.map(fileGroups, function (group, j) {
				...
				return (
					<FilePairBlock key={j} files={group}
								   ...
								   label={<StackedBlockNameLabel
									   title={fileGroups.length > 1 ? labelTitle + " " + (j + 1) : labelTitle}/>}
								   href={href}
								   isSingleItem={_.reduce(fileGroups, function (m, fp) {
									   return m + (fp || []).length;
								   }, 0) + exp.files.length + contents.length < 2 ? true : false}
					/>
				);
			}));
		}
		...
		return (
			<StackedBlock key={experimentAtId || exp.tec_rep_no || i} hideNameOnHover={false} columnClass="experiment"
				...
				<StackedBlockName relativePosition={expFxn.fileCountFromSingleExperiment(exp) > 6}>
					{experimentAtId ? <a href={experimentAtId} className="name-title">{linkTitle}</a> :
						<span className="name-title">{linkTitle}</span>}
				</StackedBlockName>
				<StackedBlockList title={listTitle} className={contentsClassName}
								  collapseLimit={collapseLimit || 3} collapseShow={collapseShow || 2}
	...
	renderBiosampleStackedBlockOfExperiments(expsWithBiosample, i) {
		...
		return (
			<StackedBlock columnClass="biosample" hideNameOnHover={false}
						  ...
						  label={<StackedBlockNameLabel title="Biosample" subtitle={bioRepTitle}
														accession={biosample.accession} subtitleVisible/>}>
				<StackedBlockName
					relativePosition={expsWithBiosample.length > 3 || expFxn.fileCountFromExperiments(expsWithBiosample) > 6}>
					{biosampleAtId ?
						<a href={biosampleAtId} className="name-title">{biosampleTitle}</a>
						: <span className="name-title">{biosampleTitle}</span>}
				</StackedBlockName>
				<StackedBlockList className="experiments" title="Experiments" collapseLimit={collapseLimit || 3}
								  collapseShow={collapseShow || 2}
		...
		return _.map(_.pairs(filesGroupedByExperimentOrGlobal).sort(function ([expAAccesion, filesForExpA], [expBAccesion, filesForExpB]) {
			...
			var nameBlock = (
				<StackedBlockName style={replicateNumbersExists ? {paddingTop: 19, paddingBottom: 19} : null}>
					{replicateNumbersExists ?
						<div>Bio Rep <b>{experiment.bio_rep_no}</b>, Tec Rep <b>{experiment.tec_rep_no}</b></div> :
						<div/>}
					{experimentAtId ? <a href={experimentAtId} className="name-title text-500">{nameTitle}</a> :
	...
	render() {
		...
		return (
			<StackedBlockTable {..._.omit(this.props, 'children', 'files')} className="expset-processed-files" fadeIn
							   allFiles={files}>
				<StackedBlockList className="sets"
								  collapseLongLists={collapseLongLists}>{experimentBlocks}</StackedBlockList>
			</StackedBlockTable>
		);
	}
		...
		if (val === 'PASS') {
			return (
				<span>
					...
					&nbsp; {linkToReport ?
					<a href={linkToReport} target="_blank" rel="noreferrer noopener">Pass</a> : "Pass"}
                </span>
			);
		} else if (val === 'FAIL') {
			return (
				<span>
					...
					&nbsp; {linkToReport ?
					<a href={linkToReport} target="_blank" rel="noreferrer noopener">Fail</a> : "Fail"}
                </span>
			);
		} else {
			return '-';
