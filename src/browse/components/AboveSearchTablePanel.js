			...
			if (resp && resp.content) {
				if (resp.content && typeof resp.content !== 'string' && resp['@type'].indexOf('StaticPage') > -1) { // Case: Static Page
					...
					if (contentSectionKeys.length > 0) {
						...
						var contentSectionToUse = _.find(resp.content, function (c) {
							return c.order === 0;
	...
	render() {
		...
		return (
			<div className="above-table-panel">
				<AboveSearchTablePanelStaticContentPane
					targetHref={this.routeStaticContentHref(this.props.href, this.props.context)}/>
			</div>
		);
	}
