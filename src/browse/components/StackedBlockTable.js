		...
		<div className={cls} key="label">
			...
			{subtitle || accession ?
				<div className={"ext" + (accession ? ' is-accession' : '')}>
					{accession ?
						<object.CopyWrapper value={accession}
											key="copy-accession">{accession || subtitle}</object.CopyWrapper>
						: subtitle}
				</div>
				: null}
		</div>
...
export class StackedBlockList extends React.PureComponent {
	...
	render() {
		...
		if (collapsibleChildrenLen > Math.min(collapseShow, 10)) { // Don't transition
			collapsibleChildrenElemsList = this.state.collapsed ? null :
...
class MultipleFileCheckbox extends React.PureComponent {
	...
	static hasCheckBox(files) {
		...
		return _.every(files, function (file) {
			return SingleFileCheckbox.hasCheckbox(file);
		});
	}
	...
	render() {
		...
		return (
			<div className="multiple-files-checkbox-wrapper inline-block" data-files-count={filesCount}
				 style={{lineHeight}}>
...
export class FileEntryBlock extends React.PureComponent {
	...
	filledFileRow() {
		...
		const cols = _.filter(columnHeaders, function (col) {
			return col.columnClass === 'file-detail';
		});
	...
	renderNameInnerTitle() {
		...
		if (!file || !fileAtId) {
			return <div key="name-title" className="name-title inline-block"><em>No file(s) or view permissions.</em>
			</div>;
		}
...
export class StackedBlockTable extends React.PureComponent {
	...
	static totalColumnsWidth = memoize(function (columnHeaders, defaultInitialColumnWidth) {
		...
		return _.reduce(origColumnWidths, function (m, v) {
			return m + v;
		}, 0);
	});
	...
	static scaledColumnWidths = memoize(function (width, columnHeaders, defaultInitialColumnWidth) {
		...
		var scale = (width / totalOrigColsWidth) || 1,
			...
			totalNewColsWidth = _.reduce(newColWidths, function (m, v) {
				return m + v;
			}, 0),
	...
	static defaultProps = {
		'columnHeaders': [
			...
			{columnClass: 'experiment', className: 'text-left', title: 'Experiment', initialWidth: 145},
			{
				...
				title: 'File Group',
				initialWidth: 40,
				visibleTitle: <i className="icon icon-download"></i>
			},
	...
	render() {
		...
		return (
			<div style={{'width': minTotalWidth}} className={
				...
				<TableHeaders {...{width, columnHeaders, defaultInitialColumnWidth, selectedFiles, allFiles}}
...
export class FileHeaderWithCheckbox extends React.PureComponent {
	...
	render() {
		...
		return (
			<React.Fragment>
				<IndeterminateCheckbox {..._.omit(this.props, 'allFiles', 'selectedFiles', 'children')} {...{
					indeterminate,
					checked
				}}
