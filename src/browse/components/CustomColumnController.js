...
export class CustomColumnSelector extends React.PureComponent {
	...
	columnDefinitionsWithHiddenState() {
		...
		return _.map(
			_.sortBy(
				_.filter(columnDefinitions, function (c) {
	...
	render() {
		return (
			<div className="row clearfix"
				 children={_.map(this.columnDefinitionsWithHiddenState(), this.renderHiddenColumnOption)}>
				{_.map(this.columnDefinitionsWithHiddenState(), (colDef, idx, all) =>
					<ColumnOption {...colDef} key={colDef.field || idx} allColumns={all} index={idx}
