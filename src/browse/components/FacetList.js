...
import {console, Filters, Schemas, layout, navigate, DateUtility} from './../../util';
...
class FacetTermsList extends React.Component {
	...
	renderTerms(terms) {
		var {facet, persistentCount, onTermClick} = this.props,
			{expanded} = this.state,
			makeTermComponent = (term) => <Term {...this.props} onClick={onTermClick} key={term.key} term={term}
												total={facet.total}/>;
		...
		if (terms.length > persistentCount) {
			...
			return (
				<div className="facet-list nav">
					<PartialList open={expanded} persistent={_.map(persistentTerms, makeTermComponent)}
	...
	render() {
		var {facet, tooltip, title} = this.props,
			...
			terms = _.filter(facet.terms, function (term) {
		...
		// List of terms
		return (
			<div className={"facet row" + (facetOpen ? ' open' : ' closed') + (facetClosing ? ' closing' : '')}
...
class Facet extends React.PureComponent {
	...
	static isStatic(facet) {
		return (
			...
			facet.total <= _.reduce(facet.terms, function (m, t) {
	...
	render() {
		...
		if (this.isStatic(facet)) {
			...
			return (
				<div
					className={"facet static row" + (selected ? ' selected' : '') + (filtering ? ' filtering' : '') + (extraClassname ? ' ' + extraClassname : '')}
					data-field={facet.field}>
					<div className="facet-static-row clearfix">
						<h5 className="facet-title">
							<span className="inline-block" data-tip={description}
								  data-place="right">&nbsp;{title}</span>
						</h5>
						<div
...
export class FacetList extends React.PureComponent {
	...
	renderFacets(maxTermsToShow = 12) {
		...
		facets = _.uniq(facets, false, function (f) {
			return f.field;
		});
