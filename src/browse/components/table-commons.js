	...
	render() {
		return (
			<div className="inline-block toggle-detail-button-container">
				<button type="button" className="toggle-detail-button"
						onClick={this.props.onClick || this.props.toggleDetailOpen}>
...
export const defaultColumnExtensionMap = {
		...
		'render': function renderDisplayTitleColumn(result, columnDefinition, props, width, popLink = false) {
			...
			if (link) { // This should be the case always
				...
				if (typeof result.email === 'string' && result.email.indexOf('@') > -1) {
					...
					title = (
						<span key="title">
                            {object.itemUtil.User.gravatar(result.email, 32, {
	...
	'@type': {
		...
		'render': function (result, columnDefinition, props, width) {
			...
			return (
				<React.Fragment>
					<i className="icon icon-fw icon-filter clickable" onClick={onClick}
					   data-tip="Filter down to this item type only."/>&nbsp;&nbsp;
					{itemTypeTitle}
				</React.Fragment>
			);
		}
	...
	'lab.display_title': {
		...
		'render': function labTitle(result, columnDefinition, props, width, popLink = false) {
			...
			var labLink;
			if (popLink) {
				labLink = <a href={object.atIdFromObject(labItem)} target='_blank'
							 rel="noopener noreferrer">{labItem.display_title}</a>;
			} else {
			...
			return (
				<span>
                    <i className="icon icon-fw icon-user-o user-icon" data-html
				...
				}}>
					{
						_.map(columnDefinitions, (colDef, i) => {
							...
							if (!colDef.noSort && typeof sortBy === 'function' && w >= 50) {
								sorterIcon = <ColumnSorterIcon sortByFxn={sortBy} currentSortColumn={sortColumn}
															   descend={sortReverse} value={colDef.field}/>;
							}
							return (
								<div
									...
									{Array.isArray(headerColumnWidths) ?
										<Draggable position={{x: w, y: 0}} axis="x"
												   onDrag={this.onAdjusterDrag.bind(this, i)}
												   onStop={this.setHeaderWidths.bind(this, i)}>
