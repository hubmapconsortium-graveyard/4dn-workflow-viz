...
export class AboveTableControls extends React.Component {
	...
	renderOverlay() {
		return (
			<Popover title="Configure Visible Columns" id="toggle-visible-columns"
					 className="toggle-visible-columns-selector">
			...
			return (
				<ChartDataController.Provider id="selected_files_section">
					<SelectedFilesControls
						...
						subSelectedFiles={filteredSelectedFiles}
		...
		return (
			<div className="pull-right right-buttons">
				<Button key="toggle-visible-columns" data-tip="Configure visible columns" data-event-off="click"
						active={this.state.open === 'customColumns'}
						onClick={this.handleOpenToggle.bind(this, 'customColumns')}>
					<i className="icon icon-fw icon-table"/>
					<i className="icon icon-fw icon-angle-down ml-03"/>
				</Button>
				<Button bsStyle="info" key="toggle-expand-layout"
						className={"expand-layout-button" + (!isFullscreen ? '' : ' expanded')}
