	...
	render() {
		...
		return (
			<div className="pull-left box selection-buttons">
				<ButtonGroup>
					<Button id="select-all-files-button" disabled={selecting || (!isAllSelected && !isEnabled)}
							className="btn-secondary" onClick={this.handleSelect}
							children={this.buttonContent(isAllSelected)}/>
				</ButtonGroup>
			</div>
		);
	}
...
export class SelectedFilesFilterByContent extends React.PureComponent {
	...
	static renderFileFormatButtonsFromBuckets(format_buckets, button_text_prefix = '', clickHandler, renderer, fileTypeFilters) {
		return _.sortBy(_.pairs(format_buckets), function (p) {
			return -p[1].length;
		}).map(function (pairs) {
...
export class SelectedFilesFilterByButton extends React.Component {
	...
	render() {
		...
		return (
			<Button id="selected-files-file-type-filter-button" className="btn-secondary" key="filter-selected-files-by"
					disabled={isDisabled}
					onClick={onFilterFilesByClick} active={currentOpenPanel === 'filterFilesBy'} data-tip={tooltip}
					data-html>
				...
				<span
					className="text-400 hidden-xs hidden-sm mr-05">File Type{currentFiltersLength === 1 ? '' : 's'}</span>
