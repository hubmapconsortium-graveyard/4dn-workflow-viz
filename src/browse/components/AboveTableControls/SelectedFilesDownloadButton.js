...
export class BrowseViewSelectedFilesDownloadButton extends React.PureComponent {
	render() {
		...
		if (subSelectedFilesCount && subSelectedFilesCount !== selectedFilesCountIncludingDuplicates) {
			...
			let tip = subSelectedFilesCount + " selected files filtered in out of " + selectedFilesCountIncludingDuplicates + " total" + (countDuplicates ? " including " + countDuplicates + " duplicates)." : '');
		}

		return (
			<SelectedFilesDownloadButton selectedFilesUniqueCount={selectedFilesUniqueCount}
										 selectedFiles={subSelectedFiles || selectedFiles} filenamePrefix="metadata_"
										 id="browse-view-download-files-btn" data-tip={tooltip} disabled={disabled}
...
export class SelectedFilesDownloadButton extends React.PureComponent {
	...
	static totalSelectedFilesCount = memoize(function (selectedFiles) {
		...
		return (
			<React.Fragment>
				...
				{this.state.modalOpen ?
					<SelectedFilesDownloadModal {...{selectedFiles, filenamePrefix, selectedFilesUniqueCount}}
		...
		return (
			<Modal show className="batch-files-download-modal" onHide={onHide} bsSize="large">
				...
				<Modal.Body>
					...
					<p>Please press the &quot;Download&quot; button below to save the metadata TSV file which contains
						download URLs and other information for the selected files.</p>
					<p>Once you have saved the metadata TSV, you may download the files on any machine or server with
					...
					<div className="extra-notes-section">
						...
						<ul className="mb-25">
							{isSignedIn ?
								<li className="mb-05">
									To download files which are not yet released, please include an <b>access key</b> in
									your cURL command which you can configure in <a href={profileHref} target="_blank"
																					rel="noopener noreferrer">your
									profile</a>.
									<br/>Use this access key in place
									of <em>{'<access_key_id>:<access_key_secret>'}</em>, above.
								</li>
								: null}
							<li className="mb-05">
								{isSignedIn ? 'If you do not provide an access key, files' : 'Files'} which do not have
								a status of &quot;released&quot; cannot be downloaded via cURL and must be downloaded
								directly through the website.
							</li>
							{foundUnpublishedFiles ?
								<li>
									For unpublished data sets, we ask that you please contact the data generating lab to
									discuss possible coordinated publication.
									In your manuscript, please cite the 4DN White Paper (<a
									href="https://doi.org/10.1038/nature23884" target="_blank"
									rel="noopener noreferrer">doi:10.1038/nature23884</a>), and please acknowledge the
									4DN lab which generated the data.
									Please direct any questions to the <a href="mailto:support@4dnucleome.org">Data
									Coordination and Integration Center</a>.
								</li>
								: null}
						</ul>
					</div>
...
function ModalCodeSnippet(props) {
	...
	return (
		<pre className="mb-15">
			...
			{isSignedIn ?
				<code style={{'opacity': 0.5}}> --user <em>{'<access_key_id>:<access_key_secret>'}</em></code> : null}
        </pre>
	);
}
	...
	render() {
		...
		return (
			<form method="POST" action="/metadata/?type=ExperimentSet&sort=accession" className="inline-block">
				...
				<Button type="submit" name="Download" bsStyle="primary"
