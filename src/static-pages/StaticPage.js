...
export const parseSectionsContent = memoize(function (context) {
	...
	return _.extend(
		{}, context, {
			'content': _.map(
				_.filter(context.content || [], function (section) {
					return section && (section.content || section.viewconfig) && !section.error;
				}),
				parse
			)
		});
	...
	renderToC() {
		...
		return (
			<div key="toc-wrapper" className={'pull-right col-xs-12 col-sm-12 col-lg-' + (12 - contentColSize)}>
				<TableOfContents pageTitle={pageTitle} fixedGridWidth={12 - contentColSize}
...
export class StaticEntry extends React.PureComponent {
	...
	render() {
			...
			return (
				<div className={outerClassName} id={id}>
					{section && section.title ?
						<HeaderWithLink className={"section-title can-collapse " + (open ? 'open' : 'closed')} link={id}
										context={context} onClick={this.toggleOpen}>
		...
		return (
			<div className={outerClassName} id={id}>
				{section && section.title ?
					<HeaderWithLink className="section-title" link={id}
									context={context}>{section.title}</HeaderWithLink>
					: null}
	...
	maybeSetRedirectedAlert() {
		...
		if (redirected_from) {
			setTimeout(function () {
				Alerts.queue({
					'title': "Redirected",
					'message': <span>You have been redirected from old page <span
						className="text-500">{redirected_from}</span> to <span
						className="text-500">{hrefParts.pathname}</span>. Please update your bookmarks.</span>,
					'style': 'warning'
				});
			}, 0);
		}
