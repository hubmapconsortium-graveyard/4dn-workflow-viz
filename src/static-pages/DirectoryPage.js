	...
	render() {
		...
		return (
			<div id="content" className="container">
				<div
					className={"static-page static-directory-page" + (childrenHaveChildren ? " directory-page-of-directories" : " leaf-directory")}
					key="wrapper">
					<DirectoryBodyGrid {...this.props} childrenHaveChildren={childrenHaveChildren}/>
					{!childrenHaveChildren ? <NextPreviousPageSection context={context} nextTitle="Next Section"
																	  previousTitle="Previous Section"/> : null}
					{content}
				</div>
			</div>
		);
...
export class DirectoryBodyGrid extends React.Component {
	...
	renderGridItem(child, index, all) {
		...
		return (
			<div className={"grid-item col-xs-12 col-md-" + (childrenHaveChildren ? '4' : '12')}
				...
				<a href={childID} className="inner">
					...
					{child.description ?
						<div
							className={"page-description" + (childrenHaveChildren ? ' text-ellipsis-container' : '')}>{child.description}</div> : null
					}
					{childrenHaveChildren && childPageCount ?
						<h6 className="section-page-count mt-07 mb-05 text-400 text-right">{childPageCount}&nbsp; <i
							className={"icon icon-fw icon-file" + (childPageCount > 1 ? "s" : "") + "-o"}/></h6> : null}
				</a>
			</div>
		);
	}
	...
	render() {
		...
		return <div className={"row grid-of-sections" + (childrenHaveChildren ? ' with-sub-children' : '')}
					children={_.map(context.children || [], this.renderGridItem)}/>;
	}
