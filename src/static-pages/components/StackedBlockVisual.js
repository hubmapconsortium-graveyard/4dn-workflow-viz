...
export class StackedBlockVisual extends React.PureComponent {
	...
	static aggregateObjectFromList(dataList, keysToShow, skipParsingKeys = null) {
		...
		_.forEach(_.keys(moreData), function (k) {
			...
			if (moreData[k].length === 0) {
				...
				if (_.any(moreData[k], function (md) {
					return md && typeof md === 'object';
				})) {
					if (!_.every(moreData[k], object.itemUtil.isAnItem)) {
						moreData[k] =
							<span className="text-600">({moreData[k].length} <span className="text-400">Objects</span>)</span>;
						return;
					}
					...
					var itemLinks = _.map(_.filter(moreData[k], function (md) {
						return md && typeof md === 'object' && md.display_title;
					}), object.itemUtil.generateLink);
					if (itemLinks && itemLinks.length > 0) remainingLength = itemLinks.length - showLength;
					moreData[k] = (
						<div>
							<span className="text-600">({itemLinks.length || moreData[k].length} <span
								className="text-400">Objects</span>)</span>
							<ol>
								{_.map(itemLinks.slice(0, showLength), (v, i) => <li key={i}>{v}</li>)}
							</ol>
							{remainingLength > 0 ?
								<div className="more-items-count"> and {remainingLength} more...</div> : null}
						</div>
					);
					return;
				}
	...
	render() {
		return (
			<div
				className={"stacked-block-viz-container" + (this.props.duplicateHeaders ? ' with-duplicated-headers' : '')}>
				{this.renderContents()}
			</div>
		);
	}
	...
	renderContents() {
		...
		if (!Array.isArray(nestedData) && nestedData) {
			...
			return _.map(leftAxisKeys, (k, idx) => {
				return <StackedBlockGroupedRow {...this.props} groupedDataIndices={columnGroups}
											   parentState={this.state} data={nestedData[k]} key={k} group={k} depth={0}
											   index={idx} toggleGroupingOpen={this.toggleGroupingOpen}/>;
			});
		} else {
...
export class StackedBlockGroupedRow extends React.PureComponent {
	...
	static collapsedChildBlocks(data, props) {
		...
		if (groupedDataIndicesPairs.length > 0) { // If columns exist, distribute these blocks by column! Otherwise (else statement @ end) we'll probably just stack em left-to-right.
					...
					return [
						...
						_.filter(_.map(allChildBlocksPerChildGroup, function (cPair) {
							if (Array.isArray(cPair[1])) {
								var res = _.filter(cPair[1], function (cBlock) {
									return listOfIndicesForGroup.indexOf(cBlock.index) > -1;
								});
				...
				inner = _.map(columnKeys, function (k) {
					...
					if (typeof props.columnSubGrouping === 'string' && props.depth <= (props.groupingProperties.length - 1)) {
						...
						if (Array.isArray(props.columnSubGroupingOrder)) {
							...
							blocksForGroup = _.map(blocksForGroupObjKeys, function (bk) {
								return [bk, blocksForGroupObj[bk]];
							});
						}
					}
					...
					return (
						<div className="block-container-group" style={containerGroupStyle}
							...
							{blocksForGroup.map(function (blockData, i) {
								...
								return <StackedBlock key={title || i} {...commonProps} {...{
									title,
									parentGrouping,
									subGrouping
								}} data={blockData}/>;
			...
			inner = _.map(allChildBlocks, ([key, data]) => <StackedBlock {...commonProps} {...{key, data}}
																		 title={key}/>);
		}

		return <div className="blocks-container" style={{'minHeight': containerGroupStyle.minHeight}}
					children={inner}/>;
	}
	...
	render() {
		var {
			groupingProperties, depth, titleMap, group, blockHeight, blockVerticalSpacing, blockHorizontalSpacing, data, groupValue,
			groupedDataIndices, index, duplicateHeaders, showGroupingPropertyTitles, checkCollapsibility
		} = this.props;
		...
		var blockHeightFull = blockHeight + blockVerticalSpacing,
			...
			childBlocks = !isOpen ? this.childBlocksCollapsed() : <div className="open-empty-placeholder" style={{
				'marginTop': partRowHeight,
				'height': rowHeight - partRowHeight,
				'marginLeft': blockHorizontalSpacing
			}}/>,
		...
		return (
			<div className={className} data-max-blocks-vertical={maxBlocksInRow}>
				<div className="row grouping-row">
					<div className="col col-xs-4 label-section"
						 style={depth === 0 && index === 0 ? {'paddingTop': Math.max(0, (isOpen ? 95 : 135) - rowHeight)} : null}>
...
export class StackedBlock extends React.PureComponent {
	...
	render() {
		...
		var blockElem = <div className={className} style={style} data-tip={tip} tabIndex={1} data-place="bottom"
							 data-html>{contents}</div>;
