...
class Announcement extends React.Component {
	...
	subtitle() {
		...
		var authorName = (author && author.display_title &&
			<span className="text-500">{author.display_title}</span>) || null;
		var unreleasedStatus = section.status && section.status !== 'released' ?
			<span className="text-500"> - {Schemas.Term.capitalizeSentence(section.status)}</span> : null;
		...
		return (
			<div className="fourDN-section-info announcement-subtitle">
				...
				{date ? <span>{!authorName ? ' Posted ' : ' '}on <DateUtility.LocalizedTime
					timestamp={date}/></span> : null}
				{unreleasedStatus}
			</div>
		);
	}
...
class AnnouncementsLoaded extends React.Component {
	...
	render() {
		if (Array.isArray(this.state.announcements)) {
			return <Announcements {...this.props} loaded={false} announcements={this.state.announcements}
								  total={this.state.totalCount}/>;
		}
		if (this.state.loading || !this.state.mounted) {
			return <h4 className="text-center mb-5" style={{'opacity': 0.5}}><i
				className="icon icon-spin icon-circle-o-notch"/></h4>;
		}
...
export class Announcements extends React.Component {
	...
	render() {
		...
		function createAnnouncement(announce, idx) {
			return <Announcement key={announce.title} index={idx} section={announce}
								 icon={collapsible ? true : false}/>;
		}
		...
		return (
			<div className={className} id={id}>{
				collapsible ? [
					<PartialList key="list" open={this.state.open} collapsible={collapsible.map(createAnnouncement)}
								 persistent={persistent.map(createAnnouncement)}/>,
					<Button key="button" bsSize="sm" className="pull-right" onClick={onSeeMoreButtonClick}
							bsStyle="default">{!this.state.open ? 'See ' + (total - persistent.length) + ' More' : 'Hide'}</Button>
