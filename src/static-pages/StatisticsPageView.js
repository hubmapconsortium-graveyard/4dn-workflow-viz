...
export default class StatisticsPageView extends React.PureComponent {
	...
	renderSubmissionsSection() {
		...
		const groupByOptions = {
			'award.project': <span><i className="icon icon-fw icon-institution mr-05"/>Project</span>
		};
		...
		if (browseBaseState !== 'all') {
			_.extend(groupByOptions, {
				...
				'experiments_in_set.experiment_type.display_title': <span><i
					className="icon icon-fw icon-bar-chart mr-05"/>Experiment Type</span>
			});
			...
			<div className="chart-section-control-wrapper row">
				<div className="col-sm-6">
					<a className={"select-section-btn" + (currentTab === 'submissions' ? ' active' : '')}
					   href="#submissions" data-tip={currentTab === 'submissions' ? null : submissionsObj.tip}
					   data-target-offset={110}>
						{submissionsObj.icon ?
							<i className={"mr-07 text-medium icon icon-fw icon-" + submissionsObj.icon}/> : null}
...
class UsageChartsCountByDropdown extends React.PureComponent {
	...
	render() {
		...
		if (chartID === 'experiment_set_views' || chartID === 'file_views') {
			menuOptions.set('views', <React.Fragment><i className="icon icon-fw icon-eye mr-05"/>Detail
				View</React.Fragment>);
			menuOptions.set('list_views', <React.Fragment><i className="icon icon-fw icon-list mr-05"/>Appearance within
				first 25 Search Results</React.Fragment>);
			menuOptions.set('clicks', <React.Fragment><i className="icon icon-fw icon-hand-o-up mr-05"/>Search Result
				Click</React.Fragment>);
		} else if (chartID === 'file_downloads') {
			menuOptions.set('download_tracking.experiment_type', <React.Fragment><i
				className="icon icon-fw icon-folder-o mr-05"/>Experiment Type</React.Fragment>);
			menuOptions.set('download_tracking.geo_country', <React.Fragment><i
				className="icon icon-fw icon-globe mr-05"/>Country</React.Fragment>);
			if (includePartialRequests) {
				...
				menuOptions.set('download_tracking.range_query', <React.Fragment><i
					className="icon icon-fw icon-television mr-05"/>Downloads as part of
					visualization</React.Fragment>);
			}
			menuOptions.set('download_tracking.file_format', <React.Fragment><i
				className="icon icon-fw icon-file-text-o mr-05"/>File Format</React.Fragment>);
		} else {
			menuOptions.set('views', <React.Fragment><i className="icon icon-fw icon-eye mr-05"/>View</React.Fragment>);
			menuOptions.set('sessions', <React.Fragment><i className="icon icon-fw icon-user mr-05"/>User
				...
				{file_downloads ?
					...
					<ColorScaleProvider resetScalesWhenChange={file_downloads}>
						...
						<AreaChartContainer {...commonContainerProps} id="file_downloads"
													...
													<div className="mb-1">
														<small>
															...
															<label className="inline-block ml-15 text-400 clickable">
																<input type="checkbox" className="mr-07"
																	   checked={includePartialRequests}
																	   onChange={onTogglePartialReqs}/>
											...
											extraButtons={
												<UsageChartsCountByDropdown {...countByDropdownProps}
																			chartID="file_downloads"
																			includePartialRequests={includePartialRequests}/>
											}>
						...
						<AreaChartContainer {...commonContainerProps} id="sessions_by_country"
											title={
												<h4 className="text-300 mt-0">
													<span
														className="text-500">{countBy.sessions_by_country === 'sessions' ? 'User Sessions' : 'Page Views'}</span> -
													by country
												</h4>
											}
											extraButtons={<UsageChartsCountByDropdown {...countByDropdownProps}
																					  chartID="sessions_by_country"/>}>
                        ...
                        */}
						...
						<AreaChartContainer {...commonContainerProps} id="fields_faceted"
											title={
												<h4 className="text-300 mt-0">
													<span
														className="text-500">Fields Faceted</span> {countBy.fields_faceted === 'sessions' ? '- by user session' : '- by search result instance'}
												</h4>
											}
											extraButtons={<UsageChartsCountByDropdown {...countByDropdownProps}
																					  chartID="fields_faceted"/>}>
			...
			<div className="stats-charts-container" key="charts" id="submissions">
				...
				{showInternalReleaseCharts ?
					...
					<ColorScaleProvider width={width} colorScale={SubmissionsStatsView.colorScaleForPublicVsInternal}>
						...
						<AreaChartContainer {...commonContainerProps} id="expsets_released_vs_internal"
											title={<h4 className="text-300 mt-0"><span className="text-500">Experiment Sets</span> -
												internal vs public release</h4>}>
				...
				<ColorScaleProvider width={width} resetScalesWhenChange={expsets_released}>
					...
					<AreaChartContainer {...commonContainerProps} id="expsets_released" title={
						<h4 className="text-300 mt-0">
							<span
								className="text-500">Experiment Sets</span> - {session ? 'publicly released' : 'released'}
						</h4>
					}>
					...
					<AreaChartContainer {...commonContainerProps} id="file_volume_released" title={
						<h4 className="text-300 mt-0">
							<span
								className="text-500">Total File Size</span> - {session ? 'publicly released' : 'released'}
						</h4>
					}>
