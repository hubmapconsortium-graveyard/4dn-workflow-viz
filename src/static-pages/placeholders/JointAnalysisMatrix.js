...
export class JointAnalysisMatrix extends React.PureComponent {
	...
	standardizeEncodeResult(result, idx) {
		...
		return _.extend({}, result, {
			...
			'state': (_.find(_.pairs(statusStateTitleMap), function (pair) {
				return pair[1].indexOf(result.status) > -1;
			}) || ["None"])[0]
		});
	}
	...
	loadSearchQueryResults() {
		...
		this.setState(
			_.object(_.map(dataSetNames, function (n) {
				return [n, null];
			})), // Reset all result states to 'null'
			() => {
	...
	render() {
		...
		if (isLoading) {
			return (
				<div>
					<div className="text-center mt-5 mb-5" style={{fontSize: '2rem', opacity: 0.5}}><i
						className="mt-3 icon icon-spin icon-circle-o-notch"/></div>
				</div>
			);
		}
			...
			<div className="static-section joint-analysis-matrix">
				<div className="row">
					<div className={"col-xs-12 col-md-" + (encode_results ? '6' : '12')}>
						...
						<h5 className="mt-0 text-500"
							style={{'marginBottom': -20, 'height': 20, 'position': 'relative', 'zIndex': 10}}>
	...
	blockPopover(data, groupingTitle, groupingPropertyTitle, props) {
		...
		var currentFilteringPropertiesVals = _.object(
			_.map(currentFilteringProperties, function (property) {
				var facetField = groupingPropertiesSearchParamMap[data_source][property],
					facetTerm = (aggrData || data)[property];
		...
		function makeSearchButton() {
			...
			return (
				<Button href={linkHref} target="_blank" bsStyle="primary" className="btn-block mt-1">View Experiment
					Sets</Button>
			);
		}
		...
		function makeSingleItemButton() {
			...
			return (
				<Button href={path} target="_blank" bsStyle="primary" className="btn-block mt-1">View Experiment
					Set</Button>
			);
		}
